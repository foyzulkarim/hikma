...
import { agentService } from '@/modules/agents/services/index.js';
...
export async function queryRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/conversation', {
    ...
    schema: {
      description: 'Process a conversation with multiple queries',
      tags: ['Query'],
  ...
  fastify.get('/history', {
    ...
    schema: {
      description: 'Get query history for user',
      tags: ['Query'],
      querystring: {
        type: 'object',
        properties: {
