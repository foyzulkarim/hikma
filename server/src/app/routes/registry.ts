...
export class RouteRegistry {
  ...
  private async loadRouteFileWithConfig(routeInfo: any): Promise<void> {
    const fileUrl = pathToFileURL(path.resolve(routeInfo.file_path)).href;
    ...
    if (module[routeInfo.exportName] && typeof module[routeInfo.exportName] === 'function') {
      ...
      if (!exportedFunction) {
        throw new Error(`No valid route function found in module: ${routeInfo.file_path}`);
      }
  ...
  private async loadRouteFile(filePath: string): Promise<void> {
    ...
    // Extract prefix from file file_path or use route name
